...
class SubTaskListCreateView(APIView,PageNumberPagination):
    ...
    def get(self,request:Request) -> Response:
        ...
        if task_title:
            filters['task__title'] = task_title

        if task_status:
            filters['task__status'] = task_status

        subtasks = SubTask.objects.filter(**filters)
...
class TaskListView(APIView):
    ...
    def get(self,request: Request) -> Response:
        ...
        filters = {}
        created_at = request.query_params.get('weekday')

        if created_at:
            filters['created_at__week_day'] = int(created_at)
